...
function StatusBadge({ status }) {
  const info = STATUS_INFO[status] || STATUS_INFO.validado;
  return (
    <span
...
export default function AprovarQtsPage() {
  ...
  const carregarValidados = useCallback(async () => {
    ...
    try {
      const resposta = await getQtsList({ page: 1, limit: 50, status: "validado" });
      setValidados(resposta.data || []);
    } catch (error) {
      setErro(error.message || "Erro ao carregar os QTS validados");
    } finally {
  ...
  useEffect(() => {
    const carregar = async () => {
      try {
        ...
        setUsuario(dados);
        if (dados?.roles?.some((r) => r.code === "aprovador")) {
  ...
  const aprovar = async (registro) => {
    ...
    try {
      ...
      setSucesso(`QTS de ${registro.dateLabel} aprovado com sucesso.`);
          ...
          {!ehAprovador ? (
            <div className="card text-center py-12">
              ...
              <p className="text-gray-600 max-w-md mx-auto mb-6">
                ...
                <strong>Aprovador</strong>. Seu perfil atual não possui
                permissão para aprovar o Quadro de Trabalho Semanal.
              </p>
              <button
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      QTS validados pendentes de aprovação da{" "}
                      {usuario?.militaryOrganization?.acronym || "sua OM"}
                    </p>
                  </div>
                </div>
                <button
                  onClick={carregarValidados}
                ...
                {carregandoLista ? (
                  ...
                  <div className="py-10 text-center">
                    ...
                    <p className="text-gray-500">
                      Nenhum QTS validado pendente de aprovação no momento.
                    </p>
                  </div>
                ) : (
